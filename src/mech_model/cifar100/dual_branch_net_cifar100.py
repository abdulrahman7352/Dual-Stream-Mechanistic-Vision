...
if __name__ == "__main__":
    ...
    # --- UPDATED LEAK-FREE DATA SPLIT FOR CIFAR-100 ---
    # Shuffle all 50,000 training indices
    shuffled_indices = torch.randperm(len(full_train_data)).tolist()
    
    # Grab 40k for training, and the remaining 10k for validation to prevent overfitting
    ...
    model = DualBranchNet(num_classes=100).to(device)

    print("=== TRAINING WITH DATA AUGMENTATION & FINE DROPOUT ===")

    best_acc = 0.0 # Track the best accuracy
    
    # ADD OPTIMIZER HERE (Outside the loop!)
    opt = torch.optim.AdamW(model.parameters(), lr=1e-4, weight_decay=5e-3)

    for epoch in range(1, 31):
        # Dynamically update the learning rate inside the optimizer
        lr = 1e-4 if epoch <= 10 else 5e-5
        for param_group in opt.param_groups:
            param_group['lr'] = lr
            
        # Pass the optimizer into train_epoch
        loss, train_acc = train_epoch(model, train_loader, opt, device, epoch)
